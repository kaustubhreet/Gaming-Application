...
{
protected:
#define MSCREEN_WIDTH 52
#define MSCREEN_HEIGHT 20
#define MINX 2
#define MINY 2
#define MAXX 49
#define MAXY 19
...
class BrickBreaker : public Base
{

    int bricks[24][2] = {
        {2, 7}, {2, 12}, {2, 17}, {2, 22}, {2, 27}, {2, 32}, {2, 37}, {2, 42}, {4, 7}, {4, 12}, {4, 17}, {4, 22}, {4, 27}, {4, 32}, {4, 37}, {4, 42}, {6, 7}, {6, 12}, {6, 17}, {6, 22}, {6, 27}, {6, 32}, {6, 37}, {6, 42}};
...
void BrickBreaker::gotoxy(int x, int y)
{
    CursorPosition.X = x;
    CursorPosition.Y = y;
...
void BrickBreaker::setcursor(bool visible, DWORD size) // set bool visible = 0 - invisible, bool visible = 1 - visible
{
    if (size == 0)
    {
...
void BrickBreaker::drawBorder()
{
    gotoxy(0, 0);
    cout << "----------------------------------------------------";
    gotoxy(0, MSCREEN_HEIGHT);
    cout << "----------------------------------------------------";

    for (int i = 0; i < MSCREEN_HEIGHT; i++)
    {
        gotoxy(0, i);
        cout << "|";
        gotoxy(MSCREEN_WIDTH, i);
        cout << "|";
    }
}

void BrickBreaker::drawBricks()
{
    for (int i = 0; i < 24; i++)
    {
...
void BrickBreaker::ballHitSlider()
{
...
void BrickBreaker::ballHitBrick()
{
    for (int i = 0; i < 24; i++)
    {
        if (visibleBricks[i] == 1)
